<script lang="ts">
  ...
  let cleanBracket: Bracket = JSON.parse(JSON.stringify(bracket));
  let firstRound = cleanBracket.gameSets[0];
  if (cleanBracket.gameSets.length > 1) {
    for (let index = 1; index < cleanBracket.gameSets[0].length; index += 2) {
      if (firstRound[index - 1].playerSets.length === 0) {
        cleanBracket.gameSets[1][Math.floor(index / 2)] = firstRound[index];
        firstRound[index] = {
          id: "empty",
          playerSets: [],
          games: [],
      ...
      } else if (firstRound[index].playerSets.length === 0) {
        cleanBracket.gameSets[1][Math.floor(index / 2)] = firstRound[index - 1];
        firstRound[index - 1] = {
          id: "empty",
          playerSets: [],
          games: [],
            ...
            >
              ...
              <div class="gameSet">
                {#each gameSet.playerSets as playerSetId}
                  <div class="playerSet">
                    <PlayerSetRender
                      playerSet={playerSetMap.get(playerSetId)}
                ...
                {#each { length: gameInfo.maxPlayerSets - gameSet.playerSets.length }}
                  <div class="invis" style={`height: ${playerSetHeight}px;`}>
                    <img
                      src={snake}
                      alt="of if you can't even say, my name (Snake)"
                    />
                  </div>
                {/each}
              </div>
