<script lang="ts">
  import { PUBLIC_API_URL } from "$env/static/public";
  import AdminBracketRender from "$lib/components/AdminBracketRender.svelte";
  import BracketRender from "$lib/components/BracketRender.svelte";
  import { onMount } from "svelte";

  let bracket: Bracket | undefined = $state();
  let view: "admin" | "user" = $state("user");

  onMount(() => {
    const urlParams = new URL(window.location.href).searchParams;
    const bracketId = urlParams.get("id") || "";

    // Fetch the bracket.
    if (bracketId !== "") {
      const BRACKET_INIT: RequestInit = {
        method: "GET",
      };
      fetch(`${PUBLIC_API_URL}/bracket/${bracketId}`, BRACKET_INIT).then(
        (response) => {
          if (response.status === 200)
            response.json().then((data) => {
              bracket = data;
            });
        },
      );
    }
  });
</script>

<main>
  {#if bracket !== undefined}
    <button
      id="toggle"
      aria-label="Toggle bracket visability"
      onclick={() => {
        if (view === "user") view = "admin";
        else view = "user";
      }}
    >
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
        <!--!Font Awesome Free v7.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2026 Fonticons, Inc.-->
        <path
          d="M320 96C239.2 96 174.5 132.8 127.4 176.6C80.6 220.1 49.3 272 34.4 307.7C31.1 315.6 31.1 324.4 34.4 332.3C49.3 368 80.6 420 127.4 463.4C174.5 507.1 239.2 544 320 544C400.8 544 465.5 507.2 512.6 463.4C559.4 419.9 590.7 368 605.6 332.3C608.9 324.4 608.9 315.6 605.6 307.7C590.7 272 559.4 220 512.6 176.6C465.5 132.9 400.8 96 320 96zM176 320C176 240.5 240.5 176 320 176C399.5 176 464 240.5 464 320C464 399.5 399.5 464 320 464C240.5 464 176 399.5 176 320zM320 256C320 291.3 291.3 320 256 320C244.5 320 233.7 317 224.3 311.6C223.3 322.5 224.2 333.7 227.2 344.8C240.9 396 293.6 426.4 344.8 412.7C396 399 426.4 346.3 412.7 295.1C400.5 249.4 357.2 220.3 311.6 224.3C316.9 233.6 320 244.4 320 256z"
        />
      </svg>
    </button>
    {#if view === "user"}
      <BracketRender {bracket} />
    {:else}
      <AdminBracketRender {bracket} />
    {/if}
  {:else}
    Bracket is loading...
  {/if}
</main>

<style>
  main {
    min-height: 100vh;
  }

  #toggle {
    position: fixed;
    bottom: 0;
    left: 0;
  }
</style>
